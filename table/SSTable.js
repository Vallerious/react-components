...
import { SimpleTextCell } from './CellTypes';
const SortTypes = {
    ASC: 'ASC',
    DESC: 'DESC',
};
...
export class SSTable extends Component {
    ...
    render() {
        ...
        let rows = this.getPaginatedItems(viewData, currentPage, perPage).map((row, i) => {
            let cells = columnDefs.map((fieldDef, idx) => {
                ...
                if (fieldDef.render && typeof fieldDef.render === "function") {
                    ...
                    cellContent = <CellTemplate data={row} field={fieldDef.field} />
                } else {
                    cellContent = <SimpleTextCell data={row} field={fieldDef.field} />
                }

                return <div className="text-center" style={{ display: 'inline-block', width: 100 / columnDefs.length + '%' }} key={idx}>
                    {cellContent}
                </div>;
            });
        ...
        return (<div className="wrapper-table" id={this.props.id}>
